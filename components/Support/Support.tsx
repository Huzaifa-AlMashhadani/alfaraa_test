...
export default function FloatingSupportChat({
    ...
    const handleSend = async () => {
        ...
        // خذ بيانات المحادثة من آخر رسالة أو الرسائل الحالية
        const lastMsg = Messages[Messages.length - 1];

        const newMsg = {
            id: Date.now(),
            conversation_id: lastMsg?.conversation_id || 1,
            sender_id: currentUser.id, // معرف المستخدم الحالي
            message: body,
        ...
        // تحديث state محليًا وتخزينه في localStorage
        setMessages((prev) => {
            const updated = [...prev, newMsg];
            localStorage.setItem("floating_support_chat_messages", JSON.stringify(updated));
            return updated;
        });
        ...
        <div className="fixed bottom-10 right-10 z-50">
            ...
            >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-12 h-12 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4-.85L3 21l1.85-3.15A7.968 7.968 0 013 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                </svg>
            </button>

            {open && (
                <div className="fixed inset-0 bg-wi/40 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="w-[700px] h-[80vh] bg-gray-200 text-white rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-gray-700" role="dialog" aria-modal="true">
                        <div className="px-6 py-4 border-b border-gray-700 flex items-center justify-between bg-gray-200">
                            <div>
                                <div className="text-xl font-semibold mb-5 text-gray-950  ">{t.header}</div>
                                <div className="text-sm text-gray-400 text-gray-700  m-5">{lang === 'en' ? 'Online' : 'متصل'}</div>
                            </div>
                            <button onClick={() => setOpen(false)} className="text-base px-3 text-gray-800 py-1 rounded hover:bg-gray-700" aria-label={t.close}>
                                {t.close}
                            </button>
                        </div>
